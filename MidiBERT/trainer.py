...
class BERTTrainer:
    def __init__(self, midibert: MidiBert, train_dataloader, valid_dataloader, 
                lr, batch, max_seq_len, mask_percent, cpu, cuda_devices=None, load_ckpt=False, stdict_o=None):
        ...
        if load_ckpt == True:
            print("\nLoad ckpt of optim")
            self.optim.load_state_dict(stdict_o)
    ...
    def iteration(self, training_data, max_seq_len, train=True):
        pbar = tqdm.tqdm(training_data, disable=False, leave=True)
